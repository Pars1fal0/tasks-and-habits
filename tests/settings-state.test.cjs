...
module.exports = [
  {
    ...
    fn() {
      const settingsState = createSettingsState({
        ...
        normalizeRemoteUserKey: (value) => String(value || "").trim().toLowerCase(),
      ...
      assert.equal(normalized.remoteSyncUserKey, "main device");
      assert.equal(remoteSafe.remoteSyncAnonKey, undefined);
      assert.equal(remoteSafe.remoteSyncUrl, undefined);
      assert.equal(remoteSafe.remoteSyncUserKey, undefined);
